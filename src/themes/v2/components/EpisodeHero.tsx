  ...
  return (
    <section className="relative pt-28 pb-16 md:pt-32 md:pb-20 bg-[#f4f2ed]">
      <div className="max-w-7xl mx-auto px-6">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-sm text-primary/50 mb-10">
          <Link href="/v2" className="hover:text-primary transition-colors">
            Home
          </Link>
          <span>/</span>
          <Link href="/v2#episodes" className="hover:text-primary transition-colors">
            Episodes
          </Link>
          <span>/</span>
          <span className="text-primary font-medium">Episode {ep.number}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="space-y-6">
            {/* Episode badge */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-secondary/15 text-primary text-xs font-bold tracking-wider uppercase">
              <span className="w-2 h-2 rounded-full bg-secondary" />
              Episode {ep.number}
            </div>

            {/* Title */}
            <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl leading-[1.05] text-primary">
              {ep.title}
            </h1>
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <a
                ...
                className="flex items-center gap-3 bg-primary text-white px-6 py-3 rounded-xl hover:bg-black transition-colors"
              >
              ...
              <a
                ...
                className="flex items-center gap-3 bg-white border border-gray-200 text-primary px-6 py-3 rounded-xl hover:bg-gray-50 transition-colors"
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="relative">
            <div className="relative w-full aspect-square max-w-[480px] mx-auto lg:ml-auto">
              ...
              {/* Image container */}
              <div className="relative w-full h-full rounded-3xl overflow-hidden border-4 border-white shadow-2xl bg-primary">
                {coverImage ? (
                  <Image
                    src={coverImage}
                    alt={ep.title}
              ...
              {/* Duration badge */}
              <div className="absolute bottom-4 right-4 bg-primary/80 backdrop-blur-sm text-white text-sm font-medium px-4 py-2 rounded-full">
                {ep.duration}
              </div>
            </div>
          </div>
